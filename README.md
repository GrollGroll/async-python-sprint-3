...
Если заходили с этого устройства - сервер опознает клиента. В ином случае предложит либо зарегеистрироваться либо зайти на сервер под существующем логином.

Для каждого нового клиента создается папка messages\personal_messages\<IP клиента> где хранятся личные переписки. 
 
Из каждого окна можно выйти командой /exit.
